...
public class ErrorManager : MonoBehaviour
{
    ...
    private string[] level = new string[] { "1 уровень", "2 уровень", "3 уровень" };
    ...
    public IEnumerator SignInPlayer(string emailreg, string passwordreg)
    {
        var signTask = auth.SignInWithEmailAndPasswordAsync(emailreg, passwordreg);
        yield return new WaitUntil(predicate: () => signTask.IsCompleted);

        if (signTask.Exception != null)
        {
            WhatErrorOut(signTask.Exception.GetBaseException() as FirebaseException);
        }
    }

    public IEnumerator RegisterPlayer(string emailsign, string passwordsign)
    {
        var registerTask = auth.CreateUserWithEmailAndPasswordAsync(emailsign, passwordsign);
        yield return new WaitUntil(predicate: () => registerTask.IsCompleted);

        if (registerTask.Exception != null)
        {
            WhatErrorOut(registerTask.Exception.GetBaseException() as FirebaseException);
        }
        else if (registerTask.Exception == null)
        {
            auth.SignInWithEmailAndPasswordAsync(emailsign, passwordsign);
            dbRef.Child("Users").Child(auth.CurrentUser.UserId).Child("1 уровень").SetValueAsync(0);
            dbRef.Child("Users").Child(auth.CurrentUser.UserId).Child("2 уровень").SetValueAsync(0);
            dbRef.Child("Users").Child(auth.CurrentUser.UserId).Child("Имя").SetValueAsync(namereg.text);
            for (int i = 0; i < 3; i++)
            {
                dbRef.Child("Leaders").Child(level[i]).Child(auth.CurrentUser.UserId).Child("Имя").SetValueAsync(namereg.text);
                dbRef.Child("Leaders").Child(level[i]).Child(auth.CurrentUser.UserId).Child("ВремяСравнение").SetValueAsync(0);
                dbRef.Child("Leaders").Child(level[i]).Child(auth.CurrentUser.UserId).Child("Время").SetValueAsync("");
                dbRef.Child("Leaders").Child(level[i]).Child(auth.CurrentUser.UserId).Child("Счет").SetValueAsync(0);
            }
            if (auth.CurrentUser != null)
            {
                error.text = "Вход выполнен: " + auth.CurrentUser.Email;
